...
## 岔題但很重要：這跟 LangChain / LangGraph / Deep Agents 差在哪？

每次講到這裡都會有人舉手：「那我為什麼不用 LangChain 就好？」

先講一句最重要的：**這四個不是同一層的東西。** 拿它們互比就像拿「螺絲」比「電鑽」比「整間工具間」。

| | 它是什麼 | 一句話比喻 |
|---|---|---|
| **LangChain** | 整合層。幾百個 loader、vectorstore、model wrapper | **零件行**。你要的螺絲它都有，但要組什麼你自己想 |
| **LangGraph** | 狀態機框架。node 是函式，edge 是控制流 | **自己焊電路板**。每條線你都畫得出來，跑起來也一定照著走 |
| **Deep Agents** | LangChain 焊好的一塊板：規劃、子代理、虛擬檔案系統、skills | **焊好的開發板**。插上就有 Claude Code 那套 pattern |
| **Claude Agent SDK** | 把整台 Claude Code 搬進你的程式裡 | **整間工具間搬過來**，連工頭、門禁、監視器都在 |

### Claude Agent SDK 贏在哪 ⭐

**① 不用 API key —— 這一項別人做不到**

它底層 spawn 的是 `claude` 這支 CLI。**CLI 讀什麼憑證，你的 agent 就讀什麼憑證** ——
所以本機 `claude login` 過，`.env` 完全空白它也會跑，吃的是**訂閱額度**。

這對上課是硬需求不是加分項：三十個學生同時跑 CRAG，沒有人會在下課後收到帳單。
LangChain / LangGraph / Deep Agents 全部都要你先掏一把 key 出來。

> **教學金句**：「別的框架第一步是『去辦一張 API key』，
> 這套的第一步是『你已經登入了，開始吧』。」

**② 內建工具是 Anthropic 幫你跑的**

`WebSearch` / `WebFetch` 你不用實作、不用付爬蟲錢、不用維護它壞掉。
在 LangGraph 裡你得自己接 Tavily / Serper 那類服務，那是另一組帳號跟另一筆錢。

這個專案的 CRAG 走 Incorrect 分支去上網查，那一行就只是 `builtin_tools=["WebSearch"]`。

**③ 權限模型預設是關的**

```python
ClaudeAgentOptions(
    tools=[],                                  # 內建工具全關 —— 預設它碰不到 Bash、碰不到你的檔案
    allowed_tools=["mcp__ragmod__search"],     # 只有這個免問直接放行
    setting_sources=[],                        # 也不要偷吃你專案的 CLAUDE.md
)
```

**預設安全，你得主動開門。** 這跟「先給全部工具再想辦法擋」是完全相反的設計。

**④ hook 是 harness 層攔截，不是叫 agent 自律**

`PreToolUse` 在工具**真的執行之前**被呼叫，回傳擋下來它就跑不了。
不是在 prompt 裡拜託模型「請不要刪檔案」—— 是模型根本沒有機會刪。

> **教學金句**：「hook 不會問 agent 同不同意 —— 它是**管 agent 的那一層**。」

**⑤ MCP 是一等公民，不是外掛**

你自己寫的七個模組，本來就是用 `create_sdk_mcp_server` 包成一個 MCP server 餵進去的。
所以接別人的 MCP server 跟寫自己的模組，**對編排來說是同一件事**（Step 9 會親手做一次）。

**⑥ 換模型是改環境變數，不是改程式碼**

`ANTHROPIC_BASE_URL` 一設就轉去 DeepSeek / Kimi / GLM / 自架 LiteLLM。
模組、policy、架構定義**一個字都不用改**。

**⑦ 它就是 Claude Code**

你在 CLI 裡學的 skills、sub-agents、plugins、hooks、CLAUDE.md，
全部可以原封不動搬進 SDK。**學一套，兩邊都會。**
其他框架你得再學一套它自己的抽象。
...
| 弱點 | 誰做得比較好 | 什麼時候會痛 |
|---|---|---|
| **沒有 checkpoint / 時間旅行** | LangGraph 每一步存一個 checkpoint，可以斷點續跑、人工審核、倒帶到任一步改狀態再重跑 | 長流程要中斷等人簽核、或要 debug「第七步為什麼走錯」 |
| **不保證照你畫的走** | LangGraph 的 edge 就是控制流，它一定照走 | 法遵、金流這種「這一步之後一定要做那一步」的場景 |
| **整合生態系薄** | LangChain 幾百個 loader / vectorstore 現成 | 要接一堆奇怪的資料來源時 |
| **調校是為 Claude** | 其他框架天生 model-agnostic | 主力模型不是 Claude 時（雖然相容端點可以接） |

> **教學金句**：「LangGraph 是**你保證流程**，Agent SDK 是**你保證準則、它保證應變**。
> 前者遇到沒想過的情況就卡住，後者遇到沒想過的情況會自己轉彎 —— 轉對轉錯看你 policy 寫得好不好。」

### 所以什麼時候選誰

| 你的情況 | 選 |
|---|---|
| 流程固定、要可稽核、要斷點續跑 | **LangGraph** |
| 要接一堆現成資料來源 | **LangChain**（當零件用，不一定要用它的 agent） |
| 想要 Claude Code 那套 pattern 但不想綁 Claude | **Deep Agents** |
| 要 agent 自己判斷、要看得見每一步、不想處理 API key | **Claude Agent SDK** ← 這門課 |

### 這門課為什麼挑 Agent SDK

1. **不用 key** —— 教室現場的硬需求
2. **hooks 讓每一步都看得見** —— 沒有它，「CRAG 比 Naive 多做了什麼」根本講不出來
3. **它就是 Claude Code** —— 學生下課回去用 CLI 的每一個技巧都接得上

**但這門課教的東西是可以搬走的。** 「模組是能力、policy 是編排、架構是資料」
這個主張跟 SDK 無關 —— 你拿去 LangGraph 實作，node 換成 tool、
policy 換成 system prompt，一樣成立。**我們教的是拆法，不是某一家的 API。**
...
這一條特別容易記錯，因為「不確定就重查」聽起來比較直覺。
現場可以用 `paper_slides` 架構讓系統自己讀 `corpus/papers/crag.pdf` 對答案 ——
**讓工具去查論文，比講師背論文可信。**
...
- 🔗 [LangGraph 持久化與時間旅行](https://langchain-ai.github.io/langgraph/concepts/persistence/) —— 看它強在哪，才知道 Agent SDK 弱在哪
- 🔗 [Deep Agents（LangChain 的 agent harness）](https://docs.langchain.com/oss/python/deepagents/overview) —— 最接近的對照組
