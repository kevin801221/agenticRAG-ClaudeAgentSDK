...
向量空間和知識圖譜各有一張**獨立的整頁**。側欄那兩格右上角有 `↗`：

- **滑過去** → 抽屜展開，看縮圖預覽
- **點下去** → 開整頁（新分頁），畫布跟著視窗大小自己縮放

它們跟旁邊三格（筆記 / 語料 / 架構）行為不一樣，所以多一個 `↗` 標出來 ——
那三格是面板，這兩格是檢視器。
...
> **這個架構抓過教材本身的錯。** 教材一度把 CRAG 的 Ambiguous 分支寫成
> 「改寫 query 再查一輪」；讓它讀 `corpus/papers/crag.pdf` 時，它指出那與論文不符 ——
> Ambiguous 是「內部精煉知識 ＋ 網路結果兩者合併」。對照原文後確認並修正了。
>
> 這就是「強制標出處」的價值：**當每一句都要指回原文，錯的那一句就藏不住。**
...
**換 store 不用重新向量化。** `data/vectors.npy` 永遠是那份唯一的真相，
chroma 只是它的另一種放法 —— 切過去的第一次啟動會從 npy 灌進去。
...
**這個規模 chroma 慢 60 倍 —— 但兩個都無關緊要。**
95% 以上的時間花在「把問題轉成向量」那一步，換 store 動不到它。

所以預設留 numpy。幾十萬筆以上、或需要增量更新 / metadata 過濾時才真的需要 chroma。
**這張表本身就是教材**：不要因為「聽起來比較專業」就裝一個向量資料庫。

### ⚠️ 重建索引之後 chroma 會自動換掉舊向量

早期版本只比「筆數一樣嗎」，所以**換 embedding 模型、或改切塊參數但筆數剛好沒變**的時候，
chroma 會安靜地繼續用舊向量 —— 檢索結果全錯，而且不會報任何錯。

現在會比對指紋（片段 id ＋ 向量內容的 sha1），不同就整份重灌。
`tests/test_retrieval.py` 有一項專門釘住這件事。
